...
class SIG17_Training_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Read HDR label
        label = read_label(
            self.image_list[index][2], "label.hdr"
        ...
        for x in range(n_w):
            for y in range(n_h):
                if (x + 1) * patch_size <= tmp_w and (y + 1) * patch_size <= tmp_h:
                    temp_patch_ldr0 = tmp_ldr0[
                        y * patch_size : (y + 1) * patch_size,
                        x * patch_size : (x + 1) * patch_size,
                    ]
                    temp_patch_ldr1 = tmp_ldr1[
                        y * patch_size : (y + 1) * patch_size,
                        x * patch_size : (x + 1) * patch_size,
                    ]
                    temp_patch_ldr2 = tmp_ldr2[
                        y * patch_size : (y + 1) * patch_size,
                        x * patch_size : (x + 1) * patch_size,
                    ]
                    ldr_patch_list.append(
                        [temp_patch_ldr0, temp_patch_ldr1, temp_patch_ldr2]
                    )
...
def SIG17_Test_Dataset(root_dir, patch_size):
    ...
    for scene in range(len(scenes_list)):
        exposure_file_path = os.path.join(
            scenes_dir, scenes_list[scene], "exposure.txt"
        )
        ldr_file_path = list_all_files_sorted(
            os.path.join(scenes_dir, scenes_list[scene]), ".tif"
        )
    ...
    for ldr_dir, label_dir, expo_times_dir in zip(
        ldr_list, label_list, expo_times_list
    ):
